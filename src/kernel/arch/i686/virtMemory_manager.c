...
{
    PTE page = (PTE)i686_physMemoryAllocBlock(); // allocate a free physical frame
    if(!page)
        return 0;
...
void i686_virtMmnger_mapPage (void* virt)
{
    ...
    {
        void* frame = i686_physMemoryAllocBlock(); // physical address of the page table
        if(!frame)
            return;
    ...
    i686_virtMmnger_allocPage(&page_table[pageEntryIndex], PTE_PAGE_PRESENT | PTE_PAGE_WRITE | PTE_PAGE_KERNEL_MODE);
}
...
void i686_virtMmnger_initialize()
{
    ...
    PDE* page_directory = i686_physMemoryAllocBlock();

    // allocate first page table
    PTE* table_0 = i686_physMemoryAllocBlock();

    // allocates 3gb page table
    PTE* table_768 = i686_physMemoryAllocBlock();
   ...
   for (int i=0, frame=0x100000, virt=0xc0000000; i<1024; i++, frame+=4096, virt+=4096)
   {
    ...
    i686_switchPDBR(page_directory);
    i686_enablePaging();    // just in case ...
    printf("Done !\n\r");
}