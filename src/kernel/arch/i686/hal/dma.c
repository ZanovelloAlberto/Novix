...
void i686_dmaEnable()
{
    i686_outb(MASTER_DMA_PORT_COMMAND_REG, DMA_COMMAND_MASK_ENABLE);
}

void i686_dmaDisable()
{
    i686_outb(MASTER_DMA_PORT_COMMAND_REG, 0x0);
}
...
void i686_dmaSetChannelCounter(uint8_t channel, uint16_t count)
{
    ...
    i686_outb(channel_port, (count) & 0xff);

    //set channel high count
    i686_outb(channel_port, (count >> 8) & 0xff);
}
...
void i686_dmaMaskChannel(uint16_t channel)
{
    if(channel >= 8)
        ...
        i686_outb(SLAVE_DMA_PORT_SINGLEMASK_REG, channel | 0b100);
    else
        i686_outb(MASTER_DMA_PORT_SINGLEMASK_REG, (channel - 4) | 0b100);
}
...
void i686_dmaUnmaskChannel(uint16_t channel)
{
    if(channel >= 8)
        ...
        i686_outb(SLAVE_DMA_PORT_SINGLEMASK_REG, channel | 0b000);
    else
        i686_outb(MASTER_DMA_PORT_SINGLEMASK_REG, (channel - 4) | 0b000);
}
...
{
    if(is_master_dma)
        i686_outb(MASTER_DMA_PORT_CLEARBYTE_FLIP_FLOP, 0x00);
    else
        i686_outb(SLAVE_DMA_PORT_CLEARBYTE_FLIP_FLOP, 0x00);
}

void i686_dmaReset(bool is_master_dma)
{
    if(is_master_dma)
        i686_outb(MASTER_DMA_PORT_MASTER_CLEAR, 0x00);
    else
        i686_outb(SLAVE_DMA_PORT_MASTER_CLEAR, 0x00);
}