...
pub const InitrdFS = struct {
    ...
    fn getRootNode(fs: *const vfs.FileSystem) *const vfs.DirNode {
        var self: *InitrdFS = @fieldParentPtr("instance", fs.instance);
    ...
    fn read(fs: *const vfs.FileSystem, file_node: *const vfs.FileNode, bytes: []u8) (Allocator.Error || vfs.Error)!usize {
        var self: InitrdFS = @fieldParentPtr("instance", fs.instance);
        const node: *const vfs.Node = @ptrCast(file_node);
...
test "open valid file" {
    ...
    try expectEqualSlices(u8, fs.opened_files.get(@ptrCast(file1)).?.name, "test1.txt");
...
test "write does nothing" {
    ...
    try expectEqualSlices(u8, fs.opened_files.get(@ptrCast(file1)).?.content, "This is a test");
}
